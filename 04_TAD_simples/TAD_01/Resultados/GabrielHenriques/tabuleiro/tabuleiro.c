...
int TemPosicaoLivreTabuleiro(tTabuleiro tabuleiro) {
    //loop para verfificar se ainda existe espaço vazo '-'
    for (int i = 0; i < TAM_TABULEIRO; i++) {
        for (int j = 0; j < TAM_TABULEIRO; j++) {
    ...
    //se nao retornar 1 entao nao tem espaço vazio e retorna 0
    return 0;
}
...
int EstaMarcadaPosicaoPecaTabuleiro(tTabuleiro tabuleiro, int x, int y, int peca) {
    if (tabuleiro.posicoes[y][x] == tabuleiro.peca1 && peca == 1) {
        return 1;
    }
    else if (tabuleiro.posicoes[y][x] == tabuleiro.peca2 && peca == 2) {
        return 1;
    }
    else {
        return 0;
