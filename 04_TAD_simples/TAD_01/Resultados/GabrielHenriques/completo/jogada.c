 ...
 */
tJogada LeJogada(){
    //inicializa o objeto jogada com entrada padrao invalidas e sucesso = 0
    tJogada jogada;
    //le os parametros fornecido pelo usuario e guarda
    printf("Digite uma posicao (x e y):\n");
    //verifica se os valores lidos foram armazenados
    //se sim sucesso recebe 1
    if (scanf("%d %d", &jogada.x, &jogada.y) == 2) {
        jogada.sucesso = 1;
        return jogada;
    }
    //caso contrario sucesso permanece 0
    else {
        jogada.sucesso = 0;
        return jogada;
    }
